...
When duplicating bidding rounds, the process also incorrectly copies the module status (e.g., showing a "filter" status from the former campaign) even when the course list was not duplicated. Because the PM can manually start the modules, the status should accurately reflect the new module's actual dates and status.
